...
from s0_utils import get_month_day_range, train_pnt, predict_pnt, \
...
FEATURE_COLS = ['Holiday', 'DayType', 'Hour', 'TimeOfYear', 'WeekOfYear',
                ...
                'annual_cos_3', 'weekly_sin_1', 'weekly_cos_1',
                'weekly_sin_2', 'weekly_cos_2', 'weekly_sin_3',
                'weekly_cos_3', 'LoadLag', 'DewPntLag', 'DryBulbLag']
...
RUN_NUM = 100
RUN_COMMENT = 'Point forecast using random forest.'
...
# columns_to_exclude = set(['Datetime', 'DEMAND', 'DewPnt', 'DryBulb', 'Zone'])
...
def main():
    ...
    train_validation_split = split_train_validation(
        whole_df, FCT_HORIZON_ALL, DATETIME_COL)
    predictions_all = []
    ...
    for i_round, train_df, validation_df in train_validation_split:
        ...
        models_all = train_pnt(train_df, config)
        predictions_df = predict_pnt(validation_df, models_all, config)
    ...
    predictions_final['APE'] = abs(predictions_final['DEMAND'] -
                                   predictions_final['predict'])/predictions_final['DEMAND']

    MAPE = predictions_final['APE'].mean()
