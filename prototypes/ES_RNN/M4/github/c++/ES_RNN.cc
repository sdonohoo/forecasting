...
struct M4TS {//storing series data
  ...
  M4TS(string category, stringstream  &line_stream) {
    array<float, NUM_OF_CATEGORIES> categories = { 0,0,0,0,0,0 }; //construct one-hot vector indicating which categories are used
...
int main(int argc, char** argv) {
  ...
  for (int ibig=0; ibig<BIG_LOOP; ibig++) { //the loop :-)
    ...
    float learning_rate= INITIAL_LEARNING_RATE; //TRAINING CODE STARTS HERE
