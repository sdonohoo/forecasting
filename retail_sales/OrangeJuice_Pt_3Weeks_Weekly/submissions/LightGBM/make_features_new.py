...
def oj_preprocess(df, aux_df, week_list, store_list, brand_list, train_df=None):
    ...
    data_filled = pd.merge(data_grid, df, how='left',
                           on=['store', 'brand', 'week'])
    ...
    data_filled['week_start'] = data_filled['week'].\
        apply(lambda x: bs.FIRST_WEEK_START + datetime.timedelta(days=(x-1)*7))

    if train_df is not None:
        data_filled = data_filled.loc[
            data_filled['week_start'] > forecast_creation_time].copy()

    return data_filled


def make_features(pred_round, train_dir, lags, window_size, offset,
                  used_columns, store_list, brand_list):
    ...
    temporal_featurizer = TemporalFeaturizer(
        df_config=df_config,
        feature_list=['month_of_year', 'week_of_month'])

    popularity_featurizer = PopularityFeaturizer(
        df_config=df_config, id_col_name='brand', data_format='wide',
        feature_col_name='price',
        wide_col_names=['price1', 'price2', 'price3', 'price4', 'price5',
                      'price6', 'price7', 'price8', 'price9', 'price10', 'price11'],
        output_col_name='price_ratio',
        return_feature_col=True)

    lag_featurizer = LagFeaturizer(df_config=df_config,
    ...
    features.rename(mapper={'move_lag_2': 'move_lag2',
                    ...
                    axis=1, inplace=True)
    features = features[[
        'store', 'brand', 'week', 'week_of_month', 'month',
        'deal',	'feat',	'move',	'price', 'price_ratio',
        'move_lag2', 'move_lag3', 'move_lag4', 'move_lag5', 'move_lag6',
        'move_lag7', 'move_lag8', 'move_lag9', 'move_lag10', 'move_lag11',
        'move_lag12', 'move_lag13', 'move_lag14', 'move_lag15', 'move_lag16',
        'move_lag17', 'move_lag18', 'move_lag19',
        'move_mean']]
