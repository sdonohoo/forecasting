...
6. Pull a Docker image from ACR using the following command:  
   ...
   ```bash
   sudo docker pull tsperf.azurecr.io/retail_sales/orangejuice_pt_3weeks_weekly/rnn_image:v1
   ```

7. Choose a name for a new Docker container (e.g. dcnn_container) and create it using command:   

   ```bash
   sudo docker run -it -v ~/Forecasting:/Forecasting --runtime=nvidia --name rnn_container tsperf.azurecr.io/retail_sales/orangejuice_pt_3weeks_weekly/rnn_image:v1
   ```
