...
def main(config_file):
    ...
    cv_setting_file = os.path.join(work_directory,
                                   config['BackTestParams']['CVSettingFile'])
    folds_per_year = config['BackTestParams']['FoldsPerYear']

    if folds_per_year not in (6, 12):
        raise Exception('Invalid folds_per_year value, {}, provided.'
                        'Valid values are 6 and 12'.format(folds_per_year))
    ...
    if folds_per_year == 6:
        ...
        for k, v in cv.train_validation_split.items():
            ...
            validation_fold_start = datetime.strptime(
                v['validation_range'][0], datetime_format)
            validation_fold_end = validation_fold_start + \
                                  relativedelta(months=1, hours=-1)
            validation_end = validation_fold_start + \
                             relativedelta(months=4, hours=-1)
            iF = 1
            round_dict = {}
            while validation_fold_end <= validation_end:
                train_fold_end = validation_fold_start + \
                                 relativedelta(weeks=-9, hours=-1)
                train_fold_end = datetime.strftime(train_fold_end,
                                                   datetime_format)
                round_dict[iF] = \
                ...
                validation_fold_start = validation_fold_start + \
                                        relativedelta(days=8)
                validation_fold_end = validation_fold_end + \
                                      relativedelta(days=8)
                iF += 1
