...
string TIME_DIR = "/home/deegup/sales_data/time"
...
int main(int argc, char** argv) {
      ...
      for (auto iter = oneChunk_vect.begin() ; iter != oneChunk_vect.end(); ++iter) {
        ...
        for (int i=INPUT_SIZE_I-1; i<(m4Obj.n- OUTPUT_SIZE_I); i++) { 
          ...
          try {
            ...
            cerr<<"caught exception 2 while doing "<<series<<endl;
            cerr << e.what() << endl;
  ...
  //calculate main method duration
  end = clock();
  double duration = ((double) (end - start)) / CLOCKS_PER_SEC;
  std::cout << "The time taken for training and forecasting is: " << duration << " seconds." << endl;

  //write main method duration to output file
  string timePath = TIME_DIR + '/time_taken' + "_" + to_string(seedForChunks) + "_"+ to_string(chunkNo)+"_"+ to_string(ibigDb)+"_LB"+ to_string(LBACK)+ ".txt";

  //save time taken to new file
  ofstream timeFile
  timeFile.open(timePath);
  timeFile << "The time taken for training and forecasting is: " << duration << " seconds." << endl;
  timeFile.close();
